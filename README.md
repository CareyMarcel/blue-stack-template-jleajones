...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create blues-stack-template-09bf
  fly apps create blues-stack-template-09bf-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app blues-stack-template-09bf
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app blues-stack-template-09bf-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'blues-stack-template-09bf-staging' does not match app name in config file 'blues-stack-template-09bf'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name blues-stack-template-09bf-db
  fly postgres attach --app blues-stack-template-09bf blues-stack-template-09bf-db

  fly postgres create --name blues-stack-template-09bf-staging-db
  fly postgres attach --app blues-stack-template-09bf-staging blues-stack-template-09bf-staging-db
  ```
